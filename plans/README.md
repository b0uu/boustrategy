...
| 039 | [Launch the public dashboard and poll live valuations every 15 minutes](039-launch-public-dashboard-and-poll-live-valuations.md) | P0 | L | none (038 DONE) | DONE (2026-09-11; live at https://boustrategy.com via named tunnel `boustrategy-public`; commits aecea6c..HEAD on `advisor/039-public-launch`; evidence in `docs/public-release.md`; benchmark feed p95 accepted as host-bound; not pushed) |
| 040 | [Make scheduled reviews research and hunt like the 2026-08-26 session](040-autonomous-research-reviews.md) | P0 | M | 039 | TODO |
...
- **Tests leak sqlite connections.** Over 20 test modules call `connect(...)` without `close()` (e.g. `tests/state/test_pipeline.py` 11 of 12 tests, `tests/reason/test_reason.py` 13 of 13, `tests/storage/test_records.py`, `tests/paper/test_paper.py`, `tests/triggers/test_triggers.py`). On Python 3.14 pytest surfaces the GC-time `ResourceWarning` as an unraisable exception attributed to whichever test is running, so a blanket `filterwarnings = ["error"]` produced seven non-deterministic failures during plan 036. S-M effort, LOW risk: wrap each test's connection in `contextlib.closing(...)` or an explicit `try/finally: conn.close()` (match `tests/reason/test_scheduler.py`, which already closes), then tighten 036's filters to a blanket `error`. Found 2026-09-08 by an executor STOP.
