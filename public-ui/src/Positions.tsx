        ...
        {data.items.length ? <>
          ...
          {data.items.map(position => <details className="position" key={position.holding_episode_id ?? position.ticker}>
            ...
            <div className="position-detail">
              ...
              <dl className="detail-grid"><Fact name="Market value">{money(position.market_value)}</Fact><Fact name="Quantity">{amount(position.shares)}</Fact><Fact name="Current price">{money(position.latest_price)}</Fact><Fact name="Average cost">{money(position.average_cost)}</Fact>{position.realization_price_low != null && <><Fact name="Fully priced at">{money(position.realization_price_low)}</Fact><Fact name="Overpriced at">{money(position.realization_price_high)}</Fact><Fact name="Invalidation price">{money(position.invalidation_price)}</Fact><Fact name="Upside to fully priced">{percent(position.upside_to_fully_priced_percent)}</Fact>{position.range_basis && <Fact name="Range basis">{position.range_basis}</Fact>}</>}<Fact name="Quote quality">{label(position.price_quality)}</Fact><Fact name="Quote time">{when(position.quote_at)}</Fact><Fact name="Asset class">{label(position.asset_class)}</Fact>{position.name && <Fact name="Theme">{label(position.theme)}</Fact>}</dl>
