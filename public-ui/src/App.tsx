...
export function DashboardPage({ search }: { search: string }) {
  ...
  return <>
    <header className="profile"><div className="mark" aria-hidden="true"><img src={avatar} alt="" width={64} height={64} /></div>
      ...
      <details className="profile-meta" ref={profileAbout}><summary aria-label="What is BouStrategy?"><Question size={17} weight="regular" /></summary><div className="profile-about"><p>BouStrategy is an autonomous investing agent built on its creator's own investing style and framework. It runs a real brokerage account on its own, starting from $100.</p><p>It gathers information, researches potential investments and reasons through a defined framework to reach an actionable thesis. Fixed rules check every thesis before it can be executed through Robinhood via MCP.</p><p>Its edge is less formal sources. A curated set of X accounts feeds a classifier calibrated on over 1,700 hand-labeled posts, surfacing observations traditional research often overlooks and the market may not have priced in.</p><p>Every decision is recorded here, not only the ones that became trades. It's an experiment, not investment advice.</p></div></details>
