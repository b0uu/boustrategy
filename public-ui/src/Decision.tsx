...
export function DecisionPage({ publicId, legacy, scope, back }: { publicId?: string; legacy?: [string, string]; scope: Scope; back: string }) {
  ...
  return <div className="trace" onClick={collapseDisclosure}>
    ...
    <SectionBoundary resetKey={data} retry={resource.refresh} name="decision trace">
      ...
      <section><details className="compact-disclosure"><summary><Chevron /> Record provenance</summary><dl className="detail-grid disclosure-body"><Fact name="Public decision"><span className="mono">{data.public_id}</span></Fact><Fact name="Review model">{data.model_provenance.requested_model ?? data.model_provenance.model_label ?? 'Not recorded'}</Fact><Fact name="Digest models">{data.model_provenance.digest_models?.length ? data.model_provenance.digest_models.join(', ') : 'Not recorded'}</Fact><Fact name="Account data model">{data.model_provenance.collector_model ?? 'Not recorded'}</Fact><Fact name="Execution model">{data.model_provenance.execution_model ?? 'Not recorded'}</Fact><Fact name="Policy version">{data.policy_evaluation.policy_version ? <span className="mono">{data.policy_evaluation.policy_version}</span> : 'Not recorded'}</Fact><Fact name="Validator version">{data.policy_evaluation.validator_version ? <span className="mono">{data.policy_evaluation.validator_version}</span> : 'Not recorded'}</Fact><Fact name="Authored schema">{data.policy_evaluation.authored_schema_version ? <span className="mono">{data.policy_evaluation.authored_schema_version}</span> : 'Not recorded'}</Fact></dl><p className="section-note">Model changes annotate one account's continuous history. They aren't separate competing portfolios.</p></details></section>
    </SectionBoundary>
