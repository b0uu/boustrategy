...
export function DecisionPage({ publicId, legacy, scope, back }: { publicId?: string; legacy?: [string, string]; scope: Scope; back: string }) {
  ...
  const orderRecorded = data.sized_order != null || data.requested_order != null || data.execution.gross_notional != null || data.execution.items.length > 0
  const collapseDisclosure = (event: MouseEvent<HTMLDivElement>) => {
    const target = event.target
    if (!(target instanceof Element) || target.closest('summary, a, button, input, select, textarea, [role="button"]')) return
    const disclosure = target.closest('details')
    if (disclosure?.open) disclosure.open = false
  }
  return <div className="trace" onClick={collapseDisclosure}>
    ...
    <SectionBoundary resetKey={data} retry={resource.refresh} name="decision trace">
      <header className="trace-head"><div className="trace-title"><h1 className="mono" tabIndex={-1}>{data.ticker}</h1><span className="action">{label(data.decision)}</span><Badge value={data.lifecycle} /><time dateTime={data.created_at}>{when(data.created_at)}</time></div>
        ...
        <dl className="trace-meta"><Fact name="Decision">{data.public_id}</Fact><Fact name="Review">{data.public_run_id ?? 'Not recorded'}</Fact><Fact name="Portfolio">{data.mode === 'paper' ? 'Paper simulation' : 'Live account'}</Fact><Fact name="Requested model">{data.model_provenance.requested_model ?? data.model_provenance.model_label ?? 'Not recorded'}</Fact><Fact name="Schema">{label(data.schema_outcome)}</Fact><Fact name="Policy">{label(data.policy_outcome)}</Fact></dl>
      ...
      <section><h2>Trigger</h2><p>{narrative?.trigger_summary ?? 'No dedicated public trigger narrative was recorded.'}</p>{data.public_run_id && <Link className="text-button" href={dashboardUrl({ scope: data.mode, tab: 'feed', run_id: data.public_run_id }, '')}>View related decisions<ArrowRight size={11} weight="bold" aria-hidden="true" /></Link>}</section>
      ...
      <section><h2>{data.mode === 'paper' ? 'Paper execution' : 'Live execution'}</h2>{orderRecorded ? <dl className="detail-grid sizing"><Fact name="Sized order">{money(data.sized_order?.notional)}</Fact><Fact name="Broker-requested notional">{money(data.requested_order?.notional)}</Fact><Fact name="Confirmed gross notional">{money(data.execution.gross_notional)}</Fact><Fact name="Confirmed quantity">{amount(data.execution.quantity)}</Fact><Fact name="Recorded fees">{money(data.execution.fees)}</Fact></dl> : <Empty>No order was recorded for this decision.</Empty>}
        {data.execution.items.length ? <details className="compact-disclosure"><summary><Chevron /> Confirmed fills</summary><div className="table-scroll" tabIndex={0} role="region" aria-label="Confirmed fills"><table><thead><tr><th>Time (ET)</th><th>Quantity</th><th>Price</th><th>Order state</th></tr></thead><tbody>{data.execution.items.map((fill, index) => <tr key={index}><td>{when(fill.occurred_at)}</td><td>{amount(fill.quantity)}</td><td>{money(fill.price)}</td><td>{label(fill.order_state)}{Number(fill.canceled_quantity) > 0 ? ` · ${amount(fill.canceled_quantity)} canceled` : ''}</td></tr>)}</tbody></table></div></details> : orderRecorded && <Empty>Confirmed fills have not been published for this decision.</Empty>}
      ...
      <section><details className="compact-disclosure"><summary><Chevron /> Portfolio management conditions</summary><div className="condition-grid">{(['add', 'trim', 'exit'] as const).map(key => <div key={key}><h3>{label(key)}</h3><TextList items={narrative?.conditions?.[key]} /></div>)}</div></details></section>
      <section><details className="compact-disclosure"><summary><Chevron /> Record provenance</summary><dl className="detail-grid disclosure-body"><Fact name="Public decision"><span className="mono">{data.public_id}</span></Fact><Fact name="Requested model">{data.model_provenance.requested_model ?? data.model_provenance.model_label ?? 'Not recorded'}</Fact><Fact name="Policy version">{data.policy_evaluation.policy_version ? <span className="mono">{data.policy_evaluation.policy_version}</span> : 'Not recorded'}</Fact><Fact name="Validator version">{data.policy_evaluation.validator_version ? <span className="mono">{data.policy_evaluation.validator_version}</span> : 'Not recorded'}</Fact><Fact name="Authored schema">{data.policy_evaluation.authored_schema_version ? <span className="mono">{data.policy_evaluation.authored_schema_version}</span> : 'Not recorded'}</Fact></dl><p className="section-note">Model changes annotate one account's continuous history. They aren't separate competing portfolios.</p></details></section>
    </SectionBoundary>
