...
import { amount, label, money, publicUrl, ruleValue, weight, when } from './format'
  ...
  return <div className="trace">
    ...
    <SectionBoundary resetKey={data} retry={resource.refresh} name="decision trace">
      <header className="trace-head"><div className="trace-title"><h1 className="mono" tabIndex={-1}>{data.ticker}</h1><span className="action">{label(data.decision)}</span><Badge value={data.lifecycle} /><time dateTime={data.created_at}>{when(data.created_at)}</time></div>
        ...
        <dl className="trace-meta"><Fact name="Portfolio">{data.mode === 'paper' ? 'Paper simulation' : 'Live account'}</Fact><Fact name="Schema">{label(data.schema_outcome)}</Fact><Fact name="Policy">{label(data.policy_outcome)}</Fact><Fact name="Model">{data.model_provenance.observed_model ?? data.model_provenance.requested_model ?? data.model_provenance.model_label ?? "Not recorded"}</Fact></dl>
      ...
      <section aria-labelledby="trace-heading"><div className="section-heading"><h2 id="trace-heading">Decision trace</h2><span>{narrative?.stages.length ?? 0} published stages</span></div>
        {narrative?.stages.length ? <div className="reasoning-chain">{narrative.stages.map((stage, index) => <details className="reasoning-stage" key={stage.stage} open={index === 0}><summary className="stage-toggle"><Chevron /><span><strong style={{ color: `var(--${stageColors[stage.stage] ?? 'text-2'})` }}>{stageNames[stage.stage] ?? label(stage.stage)}</strong><span className="stage-preview">{stage.summary}</span></span></summary><div className="stage-body"><p>{stage.summary}</p>{stage.started_at && <p className="as-of">Started {when(stage.started_at)}{stage.completed_at ? ` · Completed ${when(stage.completed_at)}` : ''}</p>}{stage.claim_ids.length > 0 && <div className="evidence-links">{stage.claim_ids.map((id, i) => <a key={id} href={`#${id}`}>Evidence {i + 1}</a>)}</div>}</div></details>)}</div> : <Empty>This historical record has a public summary, with no dedicated stage summaries.</Empty>}
      </section>
      ...
      <section><div className="section-heading"><h2>Sources &amp; claims</h2><span>{narrative?.claims.length ?? data.claims.length} published claims</span></div>
        ...
        {narrative?.sources.filter(source => !narrative.claims.some(claim => claim.source_ids.includes(source.public_id))).map(source => <div className="source-entry" key={source.public_id}>{publicUrl(source.url) ? <a href={publicUrl(source.url)!} target="_blank" rel="noopener noreferrer">{source.title} ↗</a> : source.title}<p>{source.publisher}</p></div>)}
      </section>
      ...
      <section><details className="conditions"><summary><Chevron /> Portfolio management conditions</summary><div className="condition-grid">{(['add', 'trim', 'exit', 'invalidation'] as const).map(key => <div key={key}><h3>{label(key)}</h3><TextList items={narrative?.conditions?.[key]} /></div>)}</div></details></section>
