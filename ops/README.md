...
| --- | --- | --- | --- |
| `install-digester-tasks.ps1` | `boustrategy-digester-morning`, `-midday`, `-afternoon`, `-close-halfday`, `-close`, `-weekly` | 08:45, 12:30, 14:15, 14:45, 17:45, Sunday 18:00 | `run-digester-session.ps1`: headless Codex follows the X runbook, then `app.x.run verify` checks SQLite |
| `install-runtime-tasks.ps1` | `boustrategy-review-prepare-morning`, `-midday`, `-preclose` | 09:40, 12:45, 14:45 | `run-live-prepare.ps1 -Slot <slot>`: deterministic market preparation, then a broker snapshot and the PREPARED live run through `app.broker.collector prepare-live`. The schedule revision decides whether the slot is due, so one task can carry several triggers |
| `install-runtime-tasks.ps1` | `boustrategy-review-poller-morning`, `-midday`, `-preclose` | every minute for 30 minutes from 10:00, 13:00, 15:00 | `run-review-poller.ps1 -Schedule live-<slot>`: the worker refreshes the broker snapshot through the collector before authoring and again before submission |
| `install-runtime-tasks.ps1` | `boustrategy-event-review` | every 15 minutes, 09:55 to 15:40 | `run-event-review.ps1`: `app.reason.events` starts an in-session review when crisis mode is on or a holding has a mandatory reason, away from the scheduled slots and at most three a day. A check that finds nothing starts no model session |
...
| `live-preclose` | `preclose` | 15:00 | none | last intraday entry, an hour before the close; on Fridays it answers the exposure question before the weekend |

The after-hours `live-close` schedule was retired on 2026-09-24 with a disabled revision
(`ops/live-close.retired.json`), never a pause, since a pause brakes the whole account. The
morning review handles overnight news on live prices; the 17:45 digest still feeds it.
...
Half sessions close at 13:00; the morning review still runs. Every revision keeps the 1,800-second grace, and each poller ticks for exactly that window,
...
session. The daily quota is shared: two BUY or ADD intents per day across all reviews, event
reviews included, not per review. Pausing any live schedule stops new claims on the whole account, which makes pause an
...
`data\logs\digester\`. Then enable the six `boustrategy-review-*` tasks,
`boustrategy-event-review` and `boustrategy-live-execute` together.
...
task then places approved intents, sales first. A midday review at 13:00 is usually placed by
13:30 and a pre-close review by 15:30.
